...
void setup() {
  ...
  currScalar = targetScalar = scalarOptions[0];
}
...
void draw() {
  ...
  for (int i = 0; i < amount; i++) {
    float radians = radians(i * 360.0 / amount);
    float initialX = cos(radians) * (radius + (noise(cos(radians) * radius * .005 + frameCount * .01, sin(radians) * radius * .005 - frameCount * .01) - .5) * radius * .25);
    float initialY = sin(radians) * (radius + (noise(sin(radians) * radius * .005 - frameCount * .01 + 6.43, cos(radians) * radius * .005 + frameCount * .01 - 11.19) - .5) * radius * .25);
    
    pushMatrix();
    translate(initialX, initialY);
    rotate(radians(((i - amount / 2) * currScalar)));
