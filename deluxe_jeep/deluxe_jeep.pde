...
//float[] scalarOptions = new float[]{0, 21.985073, 36.485073, 45.214672, 51.985073, 71.714676, 72.714676, 82.815246, 89.62983, 90.17334, 90.316246, 90.714676, 91.53582, 107.816246, 118.03582, 126.714676, 127.753944, 130.31622, 160.75392, 176.93164, 183.06836, 292.25394}; 
...
void draw() {
  ...
  for (int i = 0; i < amount; i++) {
    ...
    float initialX = cos(radiansVal) * (circleRadius + (noise(cos(radiansVal) * circleRadius * .005 + frameCount * .01, sin(radiansVal) * circleRadius * .005 - frameCount * .01) - .5) * circleRadius * .5);
    float initialY = sin(radiansVal) * (circleRadius + (noise(sin(radiansVal) * circleRadius * .005 - frameCount * .01 + 6.43, cos(radiansVal) * circleRadius * .005 + frameCount * .01 - 11.19) - .5) * circleRadius * .5);
